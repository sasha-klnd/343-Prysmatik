...
import { Bus, Bike, Car, Footprints, Users, Sparkles, ArrowRight, Zap, LogIn, UserPlus, LogOut, User } from 'lucide-react';
...
export function AIPromptScreen({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (prompt.trim()) onStartPlanning(prompt);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0b0d] via-[#0f1012] to-[#141518] flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-1/3 right-1/3 w-96 h-96 bg-purple-600/20 rounded-full blur-3xl animate-pulse delay-1000" />
        <div className="absolute top-1/2 right-1/4 w-64 h-64 bg-cyan-600/10 rounded-full blur-3xl animate-pulse delay-500" />
      </div>
        ...
        {isAuthenticated && userData ? (
          ...
          <>
            <motion.button
              ...
              onClick={onLogin}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-gray-300 hover:text-white text-sm font-medium transition-all backdrop-blur-xl"
            >
            ...
            <motion.button
              ...
              onClick={onSignUp}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white text-sm font-semibold transition-all shadow-lg shadow-indigo-600/20"
            >
      ...
      >
        ...
        <div className="text-center mb-12">
          ...
          {isAuthenticated && userData && (
            <motion.div
              ...
              className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600/15 border border-indigo-500/30 text-indigo-300 text-sm"
            >
              <User className="w-4 h-4" />
              Welcome back, <span className="font-semibold">{userData.name.split(' ')[0]}</span>!
              Your preferences will personalize your results.
            </motion.div>
          )}
        </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
          ...
          {[
            ...
            <motion.div key={idx}
              ...
              transition={{ delay: 0.7 + idx * 0.1, type: 'spring', stiffness: 200, damping: 15 }}
              className="relative group">
              <div className={`absolute inset-0 bg-gradient-to-br ${item.gradient} rounded-2xl blur-lg opacity-0 group-hover:opacity-50 transition-opacity duration-300`} />
          ...
          <label className="block mb-6">
            ...
            <div className="relative">
              <textarea
                ...
                rows={4}
                onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey && prompt.trim()) { e.preventDefault(); onStartPlanning(prompt); } }}
              />
            </div>
          </label>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.1 }}>
          <p className="text-sm text-gray-500 text-center mb-4 flex items-center justify-center gap-2">
            <Sparkles className="w-4 h-4" /> Popular routes in Montréal
          </p>
